...
final class ARNativeCaptureEventV2 {
  ...
  factory ARNativeCaptureEventV2.fromMap(Map<String, dynamic> map) {
    ...
    if (map.keys.any((key) => !allowedKeys.contains(key))) {
      throw const FormatException('Unexpected native capture event field.');
    }
    if (map['wireVersion'] != nativeCaptureV2WireVersion) {
    ...
    final event = ARNativeCaptureEventV2(
      kind: kind,
      attemptId: map['attemptId'] as String?,
      captureId: map['captureId'] as String?,
      captureRevision: (map['captureRevision'] as num?)?.toInt(),
      manifestId: map['manifestId'] as String?,
      reason: map['reason'] as String?,
      health: healthValue is Map
          ? ARNativeCaptureHealthV2.fromMap(
    ...
    switch (kind) {
      ...
      case ARNativeCaptureEventKindV2.recovering:
        if (event.attemptId == null || event.attemptId!.isEmpty) {
          throw const FormatException('Attempt event requires an identity.');
        }
        break;
      case ARNativeCaptureEventKindV2.committed:
        if (event.attemptId == null ||
            event.attemptId!.isEmpty ||
            event.captureId == null ||
            event.captureId!.isEmpty ||
            event.captureRevision == null ||
            event.captureRevision! < 0 ||
            event.manifestId == null ||
            event.manifestId!.isEmpty) {
          throw const FormatException('Committed terminal is incomplete.');
        }
        break;
      case ARNativeCaptureEventKindV2.abandoned:
        if (event.attemptId == null ||
          ...
          throw const FormatException('Abandoned terminal is incomplete.');
        }
        break;
      case ARNativeCaptureEventKindV2.health:
        if (event.health == null) {
          throw const FormatException('Health event requires counters.');
        }
        break;
    }
    return event;
