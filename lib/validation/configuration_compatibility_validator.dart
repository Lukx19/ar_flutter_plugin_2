...
class ConfigurationCompatibilityValidator {
  ...
  static Future<CompatibilityResult> validateConfigurations({
    ...
    try {
      ...
      await _analyzePerformanceImpact(planeConfig, captureConfig, warnings, suggestions);

      // Memory usage validation
      await _validateMemoryUsage(planeConfig, captureConfig, errors, warnings, suggestions);

      // Device-specific compatibility
      if (deviceModel != null) {
        await _checkDeviceSpecificCompatibility(deviceModel, planeConfig, captureConfig, warnings, suggestions);
      }

      // Configuration optimization check
      await _checkOptimizationOpportunities(planeConfig, captureConfig, suggestions);

      // Cross-component validation
      await _validateCrossComponentCompatibility(planeConfig, captureConfig, warnings, suggestions);
      ...
      return CompatibilityResult(
        ...
        overallScore: _calculateCompatibilityScore(errors.length, warnings.length),
      );
    } catch (e) {
      return CompatibilityResult(
        isCompatible: false,
  ...
  static Future<List<CompatibilityWarning>> checkPerformanceImpact({
    ...
    if (captureConfig.captureIntervalMs < 2000 && planeConfig == PlaneDetectionConfig.horizontalAndVertical) {
      warnings.add(CompatibilityWarning(
        type: WarningType.performance,
        severity: WarningSeverity.high,
        message: 'High-frequency capture (${captureConfig.captureIntervalMs}ms) with horizontal and vertical plane detection may cause frame rate issues',
        suggestion: 'Consider increasing capture interval to 2000ms or using only horizontal plane detection',
        affectedComponents: ['capture', 'planeDetection'],
      ));
    }

    // High resolution with intensive plane detection
    final isHighResolution = captureConfig.resolution.totalPixels > 2073600; // 1920x1080
    if (isHighResolution && planeConfig == PlaneDetectionConfig.horizontalAndVertical) {
      warnings.add(CompatibilityWarning(
        type: WarningType.performance,
        severity: WarningSeverity.medium,
        message: 'High resolution capture with horizontal and vertical plane detection may impact AR tracking performance',
        suggestion: 'Consider reducing resolution or using only horizontal plane detection',
        affectedComponents: ['capture', 'planeDetection', 'tracking'],
      ));
    }

    // RAW format with frequent capture
    if (captureConfig.format == ImageFormat.raw && captureConfig.captureIntervalMs < 5000) {
      warnings.add(CompatibilityWarning(
        type: WarningType.memory,
        severity: WarningSeverity.high,
        message: 'RAW format with frequent capture will consume significant memory and storage',
        suggestion: 'Consider using JPEG format or increasing capture interval to 5000ms+',
        affectedComponents: ['capture', 'storage'],
      ));
    }

    // Performance strategy with memory constraints
    if (captureConfig.bufferStrategy == BufferStrategy.performance && captureConfig.maxCacheSize > 15) {
      warnings.add(CompatibilityWarning(
        type: WarningType.memory,
        severity: WarningSeverity.medium,
        message: 'Performance buffer strategy with large cache may cause memory pressure',
  ...
  static Future<ARCaptureConfig> suggestOptimalCaptureConfig({
    ...
    // Base configuration
    var optimalResolution = _selectOptimalResolution(availableResolutions, planeConfig);
    ...
    if (deviceModel != null) {
      ...
      if (deviceOptimizations['reduceResolution'] == true) {
        optimalResolution = _findLowerResolution(availableResolutions, optimalResolution);
      }
  ...
  static Future<ConfigurationPerformanceImpact> assessPerformanceImpact({
    ...
    if (planeImpact > 30) {
      ...
      recommendations.add('Reduce plane detection complexity for better performance');
    }

    // Analyze resolution impact
    final resolutionImpact = _calculateResolutionImpact(captureConfig.resolution);
    performanceScore -= resolutionImpact;
    if (resolutionImpact > 20) {
      impactFactors.add('High resolution capture (${captureConfig.resolution})');
      recommendations.add('Consider lower resolution for better performance');
    }

    // Analyze capture frequency impact
    final frequencyImpact = _calculateFrequencyImpact(captureConfig.captureIntervalMs);
    performanceScore -= frequencyImpact;
    if (frequencyImpact > 15) {
      impactFactors.add('High capture frequency (${captureConfig.captureFrequencyHz.toStringAsFixed(1)}Hz)');
    ...
    final frameRateImpact = math.max(0, (100 - performanceScore) * 0.3).toDouble();
  ...
  static Future<MemoryImpact> assessMemoryImpact({
    ...
    final availableMB = availableMemoryMB?.toDouble() ?? 2048.0; // Default assumption
    final memoryPressure = estimatedUsageMB / availableMB;

    final optimizations = <String>[];
    if (memoryPressure > 0.5) {
      optimizations.add('Reduce cache size from ${captureConfig.maxCacheSize} to ${(captureConfig.maxCacheSize * 0.7).round()}');
    }
    if (captureConfig.format == ImageFormat.raw && memoryPressure > 0.3) {
      optimizations.add('Switch from RAW to JPEG format');
    }
    if (captureConfig.bufferStrategy == BufferStrategy.performance && memoryPressure > 0.4) {
  ...
  static Future<void> _checkPlatformCompatibility(
    ...
    if (!Platform.isAndroid) {
      errors.add('Capture functionality is currently only supported on Android');
      return;
    }
  ...
  static Future<void> _analyzePerformanceImpact(
    ...
    if (captureConfig.resolution.totalPixels > 8000000) { // 4K+
      performanceScore -= 30;
    } else if (captureConfig.resolution.totalPixels > 2000000) { // 1080p+
      performanceScore -= 15;
    }
    ...
    if (captureConfig.captureIntervalMs < 1000) {
      performanceScore -= 25;
    } else if (captureConfig.captureIntervalMs < 3000) {
      performanceScore -= 10;
    }
    ...
    if (performanceScore < 30) {
      warnings.add('Configuration may cause significant performance issues (score: $performanceScore/100)');
      suggestions.add('Consider reducing resolution, increasing capture interval, or limiting plane detection types');
    } else if (performanceScore < 60) {
      warnings.add('Configuration may impact performance (score: $performanceScore/100)');
  ...
  static Future<void> _validateMemoryUsage(
    ...
    if (estimatedMemoryMB > 300) {
      errors.add('Configuration requires ${estimatedMemoryMB.toStringAsFixed(1)}MB memory - likely to cause crashes');
      suggestions.add('Reduce cache size, lower resolution, or use JPEG format to reduce memory usage');
    }
    // High memory usage (may cause issues on low-end devices)
    else if (estimatedMemoryMB > 150) {
      warnings.add('Configuration requires ${estimatedMemoryMB.toStringAsFixed(1)}MB memory - may cause issues on low-end devices');
      suggestions.add('Consider testing on target devices or reducing memory usage');
    }
    // Moderate memory usage (should be noted)
    else if (estimatedMemoryMB > 50) {
      warnings.add('Configuration will use ${estimatedMemoryMB.toStringAsFixed(1)}MB memory');
    }

    // Check for memory strategy mismatch
    if (estimatedMemoryMB > 100 && captureConfig.bufferStrategy != BufferStrategy.memory) {
      suggestions.add('Consider using BufferStrategy.memory for high memory configurations');
    }
  }
  ...
  static Future<void> _checkDeviceSpecificCompatibility(
    ...
    final lowEndDevices = ['SM-A105', 'Redmi 8A', 'Galaxy A10']; // Example low-end devices
    final highEndDevices = ['Pixel 7', 'Galaxy S23', 'OnePlus 11']; // Example high-end devices

    final isLowEndDevice = lowEndDevices.any((device) => deviceModel.contains(device));
    final isHighEndDevice = highEndDevices.any((device) => deviceModel.contains(device));

    if (isLowEndDevice) {
      // More restrictive recommendations for low-end devices
      if (captureConfig.resolution.totalPixels > 1000000) { // 720p+
        warnings.add('High resolution may not perform well on this device ($deviceModel)');
        suggestions.add('Consider using 720p or lower resolution for better performance');
      }

      if (captureConfig.captureIntervalMs < 5000) {
        warnings.add('Frequent capture may impact performance on this device ($deviceModel)');
        suggestions.add('Consider increasing capture interval to 5000ms or higher');
      }

      if (planeConfig == PlaneDetectionConfig.horizontalAndVertical) {
        warnings.add('Complex plane detection may impact performance on this device ($deviceModel)');
      ...
      if (captureConfig.bufferStrategy == BufferStrategy.memory) {
        suggestions.add('This device can handle BufferStrategy.performance for better responsiveness');
      }

      if (captureConfig.resolution.totalPixels < 2000000) { // Less than 1080p
        suggestions.add('This device can handle higher resolution capture if needed');
      }
    }
  }
  ...
  static Future<void> _checkOptimizationOpportunities(
    ...
    if (!captureConfig.enableHighResCapture) {
      suggestions.add('High-resolution capture is disabled - enable it for better image quality');
    }

    if (!captureConfig.enablePoseStream && planeConfig != PlaneDetectionConfig.none) {
      suggestions.add('Pose stream is disabled but plane detection is enabled - pose stream may be useful for your use case');
    }

    // Check for suboptimal settings
    if (captureConfig.jpegQuality < 80 && captureConfig.bufferStrategy == BufferStrategy.performance) {
      suggestions.add('JPEG quality is low for performance strategy - consider increasing to 85+ for better quality');
    }

    if (captureConfig.maxCacheSize < 5 && captureConfig.captureIntervalMs < 10000) {
      suggestions.add('Cache size is small for frequent capture - consider increasing to 8+ images');
    }

    // Check for conservative settings on capable hardware
    if (captureConfig.bufferStrategy == BufferStrategy.memory && captureConfig.maxCacheSize <= 5) {
      suggestions.add('Very conservative memory settings - consider balanced strategy if device can handle it');
    }
  }
  ...
  static Future<void> _validateCrossComponentCompatibility(
    ...
    if (planeConfig == PlaneDetectionConfig.none && captureConfig.enablePoseStream) {
      warnings.add('Pose stream enabled but no plane detection - pose data may be limited');
      suggestions.add('Enable plane detection or disable pose stream to save resources');
    }

    // Check for mismatched quality levels
    if (captureConfig.format == ImageFormat.raw && captureConfig.jpegQuality < 90) {
      warnings.add('RAW format specified but JPEG quality setting is low - JPEG quality ignored for RAW');
    }

    // Check for automation conflicts
    if (captureConfig.captureIntervalMs == 0 && captureConfig.maxCacheSize > 10) {
      warnings.add('Manual capture mode with large cache size - cache may not be utilized efficiently');
      suggestions.add('Reduce cache size for manual capture or enable automatic capture');
    }
  }
  ...
  static CameraResolution _selectOptimalResolution(
    ...
    final sortedResolutions = availableResolutions
        .toList()
  ...
  static BufferStrategy _selectOptimalBufferStrategy(PlaneDetectionConfig planeConfig) {
    switch (planeConfig) {
      case PlaneDetectionConfig.horizontalAndVertical:
        return BufferStrategy.memory; // Memory-conscious for intensive plane detection
  ...
  static Map<String, dynamic> _getDeviceSpecificOptimizations(String deviceModel) {
    // This would be expanded with real device database
    final lowEndDevices = ['SM-A105', 'Redmi 8A', 'Galaxy A10'];
    final isLowEnd = lowEndDevices.any((device) => deviceModel.contains(device));
