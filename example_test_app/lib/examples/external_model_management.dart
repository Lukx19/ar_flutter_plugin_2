...
import '../../../examples/index.dart'; // Imports other custom widgets
...
class ExternalModelManagement extends StatefulWidget {
  const ExternalModelManagement({super.key, this.width, this.height});

  final double? width;
  final double? height;

  @override
  State<ExternalModelManagement> createState() =>
      _ExternalModelManagementState();
}

class _ExternalModelManagementState extends State<ExternalModelManagement> {
  // Firebase stuff
  ...
  onAnchorUploaded(ARAnchor anchor) {
    ...
    if (anchor is ARPlaneAnchor) {
      anchor.childNodes.forEach(
        (nodeName) => firebaseManager.uploadObject(
  ...
  Future<void> onDownloadButtonPressed() async {
    ...
    if (this.arLocationManager!.currentLocation != null) {
      firebaseManager.downloadAnchorsByLocation(
        ...
        this.arLocationManager!.currentLocation,
        0.1,
      );
      setState(() {
...
class FirebaseManager {
  ...
  void downloadLatestAnchor(FirebaseListener listener) {
    anchorCollection!
        ...
        .catchError(
          (error) =>
              (error) => print("Failed to download anchor: $error"),
        );
  }
