...
/// Declaration-only future iOS seam for the Proposal 08 spike worker-pull transport.
///
/// Proposal 08 spike intentionally does not implement or test Swift behavior. Android and
/// Dart own the executable reference while the future Apple implementation
/// remains a separate ticket.
protocol Proposal08PortableSurfaceStreamInterface {
...
protocol Proposal08PortableControlInterface {
...
enum Proposal08PortableInterfaceConstants {
...
struct Proposal08PortableResponseDescriptor {
    let streamToken: UInt64
...
/// Declaration-only future iOS seam for coverage renderer's selected centroid renderer.
///
...
protocol Proposal08PortableCentroidRendererInterface {
    func replaceCommittedCut(_ descriptor: Proposal08PortableRendererCutDescriptor) async throws
    func setPresentation(_ descriptor: Proposal08PortableRendererPresentationDescriptor) async throws
...
struct Proposal08PortableRendererCutDescriptor {
...
struct Proposal08PortableRendererPresentationDescriptor {
    let rendererGeneration: UInt64
    let mode: String
