...
class Uuid(val bytes: ByteArray) {
    init {
...
enum class ControlOperation(val wireValue: Int) {
    START(1), BEGIN_CHECKPOINT(2), RELEASE_CHECKPOINT(3), STOP(4);

    companion object {
        fun fromWire(value: Int): ControlOperation = entries.firstOrNull { it.wireValue == value }
...
data class ControlRequest(
    ...
    override fun equals(other: Any?): Boolean = other is ControlRequest &&
...
data class ControlResponse(
    ...
    override fun equals(other: Any?): Boolean = other is ControlResponse &&
...
data class CorrelatedControlDecode(
    val request: ControlRequest,
    val failure: ControlValidationFailure? = null,
)

/** Stable ErrorDetailV2 fields for a control failure before receipt acceptance. */
data class ControlValidationFailure(
    val errorId: Int,
    val validationPhase: Int,
    val fieldId: Int,
    val expectedValue: Long,
...
object ControlCodec {
    ...
    fun encodeRequest(request: ControlRequest): ByteArray {
    ...
    fun decodeRequest(packet: ByteArray): ControlRequest {
        ...
        val request = ControlRequest(
            operation = ControlOperation.fromWire(data.get(6).toInt() and 0xff),
            flags = data.get(7).toInt() and 0xff,
            controlRequestId = Uuid(packet.copyOfRange(16, 32)),
            sessionId = Uuid(packet.copyOfRange(32, 48)),
            captureGroupId = Uuid(packet.copyOfRange(48, 64)),
    ...
    fun decodeCorrelatedRequest(
        packet: ByteArray,
        methodOperation: ControlOperation,
    ): CorrelatedControlDecode {
        ...
        fun failure(
            ...
            request,
            ControlValidationFailure(errorId, phase, fieldId, expected, observed),
        )
        ...
        val streamTokenValid = request.streamToken >= 0 &&
            ((methodOperation == ControlOperation.START) == (request.streamToken == 0L))
        if (!streamTokenValid) {
            val expected = if (methodOperation == ControlOperation.START) 0L else 1L
            return failure(36, 3, 7, expected, request.streamToken)
        }

        return CorrelatedControlDecode(decodeRequest(packet))
    }

    fun encodeResponse(response: ControlResponse, maximumBytes: Int): ByteArray {
    ...
    fun decodeResponse(packet: ByteArray): ControlResponse {
        ...
        return ControlResponse(
            ...
            controlRequestId = Uuid(packet.copyOfRange(24, 40)),
            sessionId = Uuid(packet.copyOfRange(40, 56)),
            captureGroupId = Uuid(packet.copyOfRange(56, 72)),
    ...
    fun validateControlPayload(request: ControlRequest): ControlValidationFailure? =
        when (request.operation) {
            ControlOperation.START -> validateStartPayload(request.payload)
            ControlOperation.BEGIN_CHECKPOINT -> validateBeginCheckpointPayload(request.payload)
            ControlOperation.RELEASE_CHECKPOINT -> validateReleaseCheckpointPayload(request.payload)
            ControlOperation.STOP -> validateStopPayload(request.payload)
        }

    private fun validateStartPayload(bytes: ByteArray): ControlValidationFailure? =
        when (val detailed = StartRequestCodecV2.decodeDetailed(bytes)) {
            is StartRequestCodecV2.DetailedDecode.Valid -> null
            is StartRequestCodecV2.DetailedDecode.Invalid -> detailed.failure
        }

    private fun validateBeginCheckpointPayload(bytes: ByteArray): ControlValidationFailure? {
    ...
    private fun validateReleaseCheckpointPayload(bytes: ByteArray): ControlValidationFailure? {
        ...
        return if (legal) null else ControlValidationFailure(6, 6, 22, committed.toLong(), 2)
    }

    private fun validateStopPayload(bytes: ByteArray): ControlValidationFailure? {
        if (bytes.size != 88) return payloadLengthFailure(88, bytes.size)
        val data = ByteBuffer.wrap(bytes).order(ByteOrder.LITTLE_ENDIAN)
        val reason = data.get(0).toInt() and 0xff
        if (reason !in 1..3) return ControlValidationFailure(6, 3, 16, 3, reason.toLong())
        val mode = data.get(1).toInt() and 0xff
        if (mode > 2) return ControlValidationFailure(6, 3, 16, 2, mode.toLong())
        val flags = data.getShort(2).toInt() and 0xffff
        if (flags != 0) return ControlValidationFailure(6, 3, 5, 0, flags.toLong())
        ...
        if ((mode == 0) == checkpointEmpty) {
            return ControlValidationFailure(6, 6, 22, if (mode == 0) 1 else 0, if (checkpointEmpty) 0 else 1)
        }
        return null
    }

    private fun payloadLengthFailure(expected: Int, observed: Int) =
        ControlValidationFailure(6, 2, 3, expected.toLong(), observed.toLong())

    private fun numericFailure(expected: Long, observed: Long) =
        ControlValidationFailure(36, 4, 20, expected, observed)

    private fun reservedFailure(observed: Long) =
        ControlValidationFailure(6, 7, 5, 0, observed)

    fun encodeErrorDetail(detail: ErrorDetail): ByteArray {
    ...
    fun decodeErrorDetail(bytes: ByteArray): ErrorDetail {
    ...
    private fun validateRequest(request: ControlRequest) {
        ...
        require((request.operation == ControlOperation.START) == (request.streamToken == 0L))
    }

    private fun validateCorrelation(request: ControlRequest) {
        validateOrdinal(request.sessionGeneration, "sessionGeneration")
        validateOrdinal(request.groupGeneration, "groupGeneration")
        validateOrdinal(request.coverageEpoch, "coverageEpoch")
    ...
    private fun validateResponse(response: ControlResponse) {
