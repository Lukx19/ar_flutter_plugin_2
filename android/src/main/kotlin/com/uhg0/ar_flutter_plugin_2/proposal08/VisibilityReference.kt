...
object PictureViewBins24 {
    ...
    val centers: List<Q15Vector> = listOf(
        Q15Vector(-9346, -21845, -22564),
        Q15Vector(-22564, -21845, -9346),
        Q15Vector(-22564, -21845, 9346),
        Q15Vector(-9346, -21845, 22564),
        Q15Vector(9346, -21845, 22564),
        Q15Vector(22564, -21845, 9346),
        Q15Vector(22564, -21845, -9346),
        Q15Vector(9346, -21845, -22564),
        Q15Vector(-12539, 0, -30273),
        Q15Vector(-30273, 0, -12539),
        Q15Vector(-30273, 0, 12539),
        Q15Vector(-12539, 0, 30273),
        Q15Vector(12539, 0, 30273),
        Q15Vector(30273, 0, 12539),
    ...
    fun classify(vector: Q15Vector): Int {
    ...
    fun normalize(x: Int, y: Int, z: Int): Q15Vector {
...
class VisibilityCoverage24 {
    ...
    fun credit(bin: Int, amount: Long = 1) {
        ...
        packed[bin] = exact.coerceAtMost(PictureViewBins24.PACKED_COUNT_MAXIMUM.toLong()).toInt()
    ...
    fun evaluate(
        ...
        val candidates = mutableListOf<VisibilityTriplet>()
            ...
            for (second in first + 1 until 23) {
                for (third in second + 1 until 24) {
                    ...
                    for (index in indices) {
                        val dot = normal.dot(PictureViewBins24.centers[index])
                        if (normalConfidence < 64) {
                            incidence += 65535
                        } else if (dot < PictureViewBins24.FRONT_DOT) {
                            eligible = false
                            break
                        } else {
                            incidence += roundHalfUp(
                                65535L * (dot - PictureViewBins24.FRONT_DOT),
                                PictureViewBins24.NEAR_DOT - PictureViewBins24.FRONT_DOT,
                    ...
                    val nearNormal = normalConfidence < 64 ||
                        indices.any {
                            normal.dot(PictureViewBins24.centers[it]) >=
                                PictureViewBins24.NEAR_DOT
                        }
                    candidates += VisibilityTriplet(
                        indices = indices,
                        score = score,
    ...
    private fun isDiverse(indices: List<Int>): Boolean {
        for (left in indices.indices) {
            for (right in left + 1 until indices.size) {
                if (
                    PictureViewBins24.centers[indices[left]].dot(
                        PictureViewBins24.centers[indices[right]],
                    ) > PictureViewBins24.DIVERSE_DOT
                ) return false
            }
        }
        return true
    ...
    private fun hasNearNormal(
        triplet: VisibilityTriplet,
        normal: Q15Vector,
        normalConfidence: Int,
    ): Boolean = normalConfidence < 64 || triplet.indices.any {
        normal.dot(PictureViewBins24.centers[it]) >= PictureViewBins24.NEAR_DOT
    }
