...
interface PersistentFusionSession {
    ...
    fun admit(observations: Iterable<VoxelObservation>)
    fun forEachCanonical(consumer: (CanonicalSurface) -> Unit)
    fun replayPicture(pictureId: Int, affectedSurfaceCount: Int): Long
}

abstract class CompactPersistentSession(
    ...
    final override fun admit(observations: Iterable<VoxelObservation>) {
    ...
    protected abstract fun admitAt(slot: Int, observation: VoxelObservation)
    protected abstract fun isVisible(slot: Int): Boolean
    protected abstract fun weight(slot: Int): Int

    override fun forEachCanonical(consumer: (CanonicalSurface) -> Unit) {
    ...
    protected fun canonical(
        ...
        return CanonicalSurface(
            ...
            key = VoxelKey(surfaces[base + X], surfaces[base + Y], surfaces[base + Z]),
    ...
    protected fun slotOf(key: VoxelKey): Int {
        var bucket = hash(key)
        repeat(HASH_SLOTS) {
    ...
    private fun findOrInsert(observation: VoxelObservation): Int {
    ...
    companion object {
        ...
        private fun canonicalChecksum(surface: CanonicalSurface): Long =
...
open class SignedPersistentSession(
    candidateId: String,
    surfaceCapacity: Int,
...
) : CompactPersistentSession(candidateId, surfaceCapacity, associationCapacity, maximumLineageIds) {
    override fun admitAt(slot: Int, observation: VoxelObservation) {
        val base = slot * SURFACE_LANES
...
class PlanarPersistentSession(
    candidateId: String,
    surfaceCapacity: Int,
    associationCapacity: Int,
    maximumLineageIds: Int,
) : SignedPersistentSession(
    candidateId,
    surfaceCapacity,
    associationCapacity,
    maximumLineageIds,
    ...
    override fun forEachCanonical(consumer: (CanonicalSurface) -> Unit) {
        ...
        repeat(surfaceCount) { slot ->
            ...
            val key = VoxelKey(surfaces[base + X], surfaces[base + Y], surfaces[base + Z])
            val members = listOf(
                slot,
                slotOf(VoxelKey(key.x + 1, key.y, key.z)),
                slotOf(VoxelKey(key.x, key.y + 1, key.z)),
                slotOf(VoxelKey(key.x + 1, key.y + 1, key.z)),
            )
...
data class PersistentCampaignResult(
    val candidateId: String,
...
class PersistentKernelHarness(
    private val kernel: FusionKernel,
    private val allocatedBytes: () -> Long,
    private val cpuNanos: () -> Long,
) {
    fun execute(): PersistentCampaignResult {
        ...
        return PersistentCampaignResult(
    ...
    private fun boundaryObservations(start: Int, count: Int): Iterable<VoxelObservation> = Iterable {
        object : Iterator<VoxelObservation> {
            var index = 0
            override fun hasNext(): Boolean = index < count * 2
            override fun next(): VoxelObservation {
                val logical = start + index / 2
                val duplicate = index++ and 1
                val key = keyFor(logical)
                return VoxelObservation(key.x, key.y, key.z, 1, logical * 2 + duplicate)
            }
        }
    }
    ...
    private fun capacityOverflow(): Int {
        ...
        session.admit(listOf(VoxelObservation(0, 0, 0, 2, 1), VoxelObservation(10, 0, 0, 2, 2)))
    ...
    private fun lineageOverflow(): Int {
        ...
        session.admit(List(LINEAGE_LIMIT + 1) { VoxelObservation(0, 0, 0, 1, it) })
        return session.overflowCount
    }

    private fun checkedOverflowFailures(): Int {
        val session = kernel.persistentSession(1, 1, LINEAGE_LIMIT)
        session.admit(listOf(VoxelObservation(0, 0, 0, Int.MAX_VALUE, 1)))
        return if (session.overflowCount == 1) 0 else 1
    }

    private fun keyFor(index: Int): VoxelKey {
        val patch = index / 4
        return VoxelKey(patch * 2 + index % 2, (index % 4) / 2, 0)
    }
