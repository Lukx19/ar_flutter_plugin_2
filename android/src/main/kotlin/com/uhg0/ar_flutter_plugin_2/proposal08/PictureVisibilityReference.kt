...
enum class PictureVisibilityOccupancy { CONFIRMED, AMBIGUOUS, ABSENT }

data class PictureVisibilityCamera(
    val imageWidth: Int,
    val imageHeight: Int,
    val fxQ8: Int,
    val fyQ8: Int,
    val cxQ8: Int,
    val cyQ8: Int,
    val groupFromCameraTranslationMm: VoxelKey,
...
data class PictureVisibilitySurface(
    val surfaceId: Long,
    val key: VoxelKey,
    val normal: Q15Vector,
    val normalConfidence: Int,
    val published: Boolean = true,
    val occlusionEligible: Boolean = true,
    val occupancy: PictureVisibilityOccupancy = PictureVisibilityOccupancy.CONFIRMED,
    val semanticRevision: Long = 1,
    val alreadyCredited: Boolean = false,
)

data class PictureVisibilityCutState(
...
enum class PictureVisibilityRejection {
    APPROVED,
    SCOPE_STALE,
    CUT_INCOMPATIBLE,
...
data class PictureVisibilityEvaluation(
    val rejection: PictureVisibilityRejection,
    val bin: Int? = null,
    val surfaceCenterMm: VoxelKey? = null,
    val cameraPointMm: VoxelKey? = null,
    val depthMm: Int? = null,
    val projectedUQ8: Int? = null,
    val projectedVQ8: Int? = null,
    val footprintQ16: Long = 0,
    val facingQ15: Int = 0,
    val viewDirection: Q15Vector? = null,
) {
    val approved: Boolean get() = rejection == PictureVisibilityRejection.APPROVED
}

object PictureVisibilityEvaluator {
    fun evaluate(
        camera: PictureVisibilityCamera,
        surface: PictureVisibilitySurface,
        cut: PictureVisibilityCutState = PictureVisibilityCutState(),
        occludingCells: Iterable<VoxelKey> = emptyList(),
    ): PictureVisibilityEvaluation {
        if (cut.scopeStale) return reject(PictureVisibilityRejection.SCOPE_STALE)
        if (cut.cutIncompatible) return reject(PictureVisibilityRejection.CUT_INCOMPATIBLE)
        if (cut.historyIndeterminate) return reject(PictureVisibilityRejection.HISTORY_INDETERMINATE)
        if (cut.coveragePending) return reject(PictureVisibilityRejection.COVERAGE_PENDING)
        if (!surface.published || surface.normalConfidence !in 0..255 ||
            surface.occupancy == PictureVisibilityOccupancy.ABSENT
        ) return reject(PictureVisibilityRejection.SURFACE_INELIGIBLE)
        if (!camera.isValid) return reject(PictureVisibilityRejection.CAMERA_MODEL_UNSUPPORTED)
        if (!camera.poseValid) return reject(PictureVisibilityRejection.POSE_INVALID)

        val surfaceCenter = VoxelKey(
        ...
        val viewDirection = try {
            ...
            return reject(
                PictureVisibilityRejection.DIRECTION_DEGENERATE,
        ...
        if (depth < camera.nearMm || depth > camera.farMm) {
            return reject(
                PictureVisibilityRejection.DEPTH_RANGE,
                surfaceCenter,
                cameraPoint,
                depth,
        ...
        if (projectedU !in 0 until camera.imageWidth * 256 ||
            ...
            return reject(
                PictureVisibilityRejection.FRUSTUM_OUTSIDE,
                surfaceCenter,
                cameraPoint,
                depth,
                projectedU,
        ...
        if (footprint < 64L * 65536L) {
            return reject(
                PictureVisibilityRejection.FOOTPRINT_TOO_SMALL,
                surfaceCenter,
                cameraPoint,
                depth,
                projectedU,
        ...
        if (surface.normalConfidence >= 64 &&
            surface.normal.dot(viewDirection) < PictureViewBins24.FRONT_DOT
        ) {
            return reject(
                PictureVisibilityRejection.BACKSIDE,
                surfaceCenter,
                cameraPoint,
                depth,
                projectedU,
        ...
        if (surface.occlusionEligible &&
            SupercoverCells100mm.cellsBetween(cameraCenter, surfaceCenter).any { it in occluding }
        ) {
            return reject(
        ...
        if (surface.alreadyCredited) {
            return reject(
                PictureVisibilityRejection.CAPTURE_SURFACE_DUPLICATE,
                surfaceCenter,
                cameraPoint,
                depth,
                projectedU,
        ...
        return PictureVisibilityEvaluation(
            rejection = PictureVisibilityRejection.APPROVED,
            bin = PictureViewBins24.classify(viewDirection),
            surfaceCenterMm = surfaceCenter,
    ...
    private fun transform(camera: PictureVisibilityCamera, point: VoxelKey): VoxelKey {
    ...
    private fun reject(
        rejection: PictureVisibilityRejection,
        surfaceCenterMm: VoxelKey? = null,
        cameraPointMm: VoxelKey? = null,
        depthMm: Int? = null,
        projectedUQ8: Int? = null,
        projectedVQ8: Int? = null,
        footprintQ16: Long = 0,
        facingQ15: Int = 0,
        viewDirection: Q15Vector? = null,
    ) = PictureVisibilityEvaluation(
        rejection,
...
object SupercoverCells100mm {
    fun cellsBetween(startMm: VoxelKey, endMm: VoxelKey): Set<VoxelKey> {
        val start = VoxelKey(
        ...
        val touched = linkedSetOf<VoxelKey>()
        var guard = 0
        while (guard++ < 100_000) {
            ...
            for (mask in 1 until (1 shl tied.size)) {
                ...
                touched += VoxelKey(cell[0], cell[1], cell[2])
            }
            tied.forEach { axis ->
...
data class GuidanceCandidateInput(
    val surfaceId: Long,
    val evaluation: PictureVisibilityEvaluation,
    val normal: Q15Vector,
    val count: Int,
    val occupancy: PictureVisibilityOccupancy,
)

data class GuidanceEnvironment(
    val gridMinMm: VoxelKey,
    val gridMaxMm: VoxelKey,
    val occupiedCells: Set<VoxelKey>,
)
...
data class GuidanceTarget(
    ...
    val bin: Int,
    val gainQ16: Int,
    val standpointMm: VoxelKey,
)

object GuidanceReference {
    fun select(
        candidates: Iterable<GuidanceCandidateInput>,
        environment: GuidanceEnvironment? = null,
    ): List<GuidanceTarget> =
        candidates.mapNotNull { candidate ->
            ...
            val occupancyWeight = when (candidate.occupancy) {
                PictureVisibilityOccupancy.CONFIRMED -> 65535
                PictureVisibilityOccupancy.AMBIGUOUS -> 32768
                PictureVisibilityOccupancy.ABSENT -> 0
            }
            val footprintWeight = roundTiesEven(65535L * evaluation.footprintQ16, 256L * 65536L)
                .coerceIn(0, 65535)
            val incidence = roundTiesEven(
                candidate.normal.dot(PictureViewBins24.centers[bin]),
                32767,
            ).coerceIn(0, 65535)
            var gain = 65535L
            ...
            val direction = PictureViewBins24.centers[bin]
            val standpoint = VoxelKey(
        ...
        }.sortedWith(compareByDescending<GuidanceTarget> { it.gainQ16 }
            .thenBy { it.surfaceId }
        .thenBy { it.bin })
        .take(3)
    ...
    private fun validStandpoint(
        ...
        environment.occupiedCells.forEach { cell ->
            val occupied = VoxelKey(100 * cell.x + 50, 100 * cell.y + 50, 100 * cell.z + 50)
        ...
        return SupercoverCells100mm.cellsBetween(standpoint, target)
