 ...
 */
internal class LegacyCanonicalState(
    val group: SurfaceGroup,
    val nextHighWater: Long,
    val resident: CompactResident,
    val sourceCount: Int,
    val supportCount: Int,
    val lineageCount: Int,
    val geometryRevision: Long,
    val lineageRevision: Long,
    val baseline: committedEmptyBaseline?,
    val sourceHash: ByteArray,
    private val sourceCursor: ((PagedSource) -> Unit) -> Unit,
    private val supportCursor: ((Long, PagedSource) -> Unit) -> Unit,
    private val sourceLookup: (Long) -> PagedSource?,
    private val explicitSourceIndex: LegacySourceIndex?,
    private val canonicalReceiptCursor: ((LegacyCanonicalReceipt) -> Unit) -> Unit,
) {
    fun visitSources(visitor: (PagedSource) -> Unit) = sourceCursor(visitor)

    fun visitSupports(visitor: (target: Long, source: PagedSource) -> Unit) =
        supportCursor(visitor)

    fun sourceById(id: Long): PagedSource? = sourceLookup(id)

    fun sourceIndexReceipt(): LegacySourceIndexReceipt? = explicitSourceIndex?.receipt()

    /** Reopens immutable legacy receipt evidence one entry at a time. */
    fun visitCanonicalReceipts(visitor: (LegacyCanonicalReceipt) -> Unit) =
 ...
 */
internal class LegacyCanonicalReceipt(
...
internal data class LegacySourceIndexReceipt(
    val records: Int,
...
internal class LegacySourceIndex private constructor(
    ...
    fun find(reader: RandomAccessFile, id: Long): PagedSource? {
        lookups++
        var probes = 0
        var low = 0
    ...
    fun visit(visitor: (PagedSource) -> Unit) {
    ...
    fun receipt() = LegacySourceIndexReceipt(
    ...
    companion object {
        ...
        fun build(file: File, sourceOffset: Long, count: Int): LegacySourceIndex {
            ...
            return LegacySourceIndex(file, sourceOffset, order, reads)
        }

        private fun readSourceRecord(reader: RandomAccessFile): PagedSource =
            SurfaceOwnershipLegacyCodec.readSourceRecord(reader)
    }
}

/** Bounded, decoder-only bridge for immutable v1-v5 files. */
internal object SurfaceOwnershipLegacyCodec {
    ...
    ): LegacyCanonicalState =
        try {
            val prefix = pageSha256(group.value.encodeToByteArray()).hex()
            val snapshot = File(directory, "canonical-surface-surface-$prefix.snapshot")
            val ledger = File(directory, "canonical-surface-surface-$prefix.ledger")
    ...
    private fun decodeSnapshot(
        ...
        var baseline: committedEmptyBaseline? = null
        ...
        snapshotInput(snapshot).use { counted ->
            ...
            repeat(rowCount) { index ->
                ...
                val y = data.readInt()
                val z = data.readInt()
                val region = StorageRegion(data.readInt(), data.readInt(), data.readInt())
                ...
                val location = CompactLocation(configuration, Voxel(x, y, z))
        ...
        val explicitSourceIndex =
            if (explicitSources && !finalSourcesSorted)
                LegacySourceIndex.build(snapshot, finalSourceOffset, finalSourceCount)
            else null
        val sourceLookup: (Long) -> PagedSource? = { id ->
            if (explicitSources)
        ...
        val sourceCursor: ((PagedSource) -> Unit) -> Unit = { visitor ->
            if (explicitSources) {
                if (finalSourcesSorted) {
        ...
        val supportCursor: ((Long, PagedSource) -> Unit) -> Unit = { visitor ->
            if (version == 1) {
                repeat(rowCount) { order ->
                    val slot = idOrder[order]
                ...
                RandomAccessFile(snapshot, "r").use { supportReader ->
                    RandomAccessFile(snapshot, "r").use { sourceReader ->
                        ...
                        var cachedSource: PagedSource? = null
        ...
        supportCursor { _, _ -> }
        val canonicalReceiptCursor: ((LegacyCanonicalReceipt) -> Unit) -> Unit = { visitor ->
            when (version) {
    ...
    private fun validateLedger(file: File, group: SurfaceGroup): Long {
            ...
            repeat((file.length() / LEDGER_RECORD_BYTES).toInt()) {
                ...
                require(
                    recordRevision == ++revision &&
                        ...
                        hash.contentEquals(pageSha256(body)) &&
    ...
    private fun skipCanonicalReceipts(
        ...
        repeat(count) {
            ...
            if (version >= 4) {
                ...
                val kind = bounded(data.readInt(), 0, CanonicalOperation.entries.lastIndex)
                val targets = FieldDigest()
                val targetCount = bounded(data.readInt(), 0, configuration.surfaceCapacity)
                repeat(targetCount) {
                    val id = data.readLong(); val ownerGroup = data.readUTF()
                    val voxel = Voxel(data.readInt(), data.readInt(), data.readInt())
                    val region = StorageRegion(data.readInt(), data.readInt(), data.readInt())
                    ...
                    require(CompactLocation(configuration, voxel) == CompactLocation(region, page))
                ...
                if (version >= 3) {
                    ...
                    validateCanonicalReceipt(
                        ...
                        lineageRevision,
                        InlineCanonicalReceipt(command, kind, receiptGeometry, receiptLineage,
    ...
    private fun validateCanonicalReceipt(
        ...
        geometryRevision: Long,
        lineageRevision: Long,
        expected: InlineCanonicalReceipt? = null,
    ): CanonicalReceiptHeader {
        val body = DataInputStream(LegacyLimitedInputStream(source, bytes.toLong()))
        ...
        val kind = bounded(body.readInt(), 0, CanonicalOperation.entries.lastIndex)
        ...
        repeat(targetCount) {
            ...
            val voxel = Voxel(body.readInt(), body.readInt(), body.readInt())
            val region = StorageRegion(body.readInt(), body.readInt(), body.readInt())
            val page = body.readInt()
            val normal = body.readInt()
            val confidence = body.readInt()
            require(
                CompactLocation(configuration, voxel) == CompactLocation(region, page) &&
                    normal in 0..0xffff &&
                    confidence in 0..255 &&
                    (normal ushr 8) != 0x80 &&
                    (normal and 0xff) != 0x80
        ...
        return CanonicalReceiptHeader(geometry, lineage, high, live)
    }
        ...
        at(file, offset).use { data ->
            repeat(bounded(data.readInt(), 0, configuration.transactionCapacity)) {
                ...
                if (version >= 4) {
                    ...
                    data.readUTF()
                    bounded(data.readInt(), 0, CanonicalOperation.entries.lastIndex)
    ...
    private fun scanStoredCanonicalReceipts(
        ...
        geometryRevision: Long,
        lineageRevision: Long,
        visitor: (LegacyCanonicalReceipt) -> Unit,
    ) {
        FileInputStream(file).use { input ->
            input.channel.position(offset)
            val data = DataInputStream(input)
            repeat(bounded(data.readInt(), 0, configuration.transactionCapacity)) {
                val commandHash = CanonicalReceiptBytes(ByteArray(32).also(data::readFully))
                val fingerprint = CanonicalReceiptBytes(ByteArray(32).also(data::readFully))
                ...
                val body = DataInputStream(java.security.DigestInputStream(
                    LegacyLimitedInputStream(input, length.toLong()), digest,
                ))
                ...
                visitor(
                    LegacyCanonicalReceipt(
                        ...
                        FileInputStream(file).use { source ->
                            ...
                            require(CanonicalReceiptBytes(copied.digest()) == canonicalHash)
                        }
                    },
                )
            }
            ...
            repeat(bounded(data.readInt(), 0, configuration.transactionCapacity)) {
                ...
                visitor(
                    LegacyCanonicalReceipt(
                        ...
                        require(actual == length && CanonicalReceiptBytes(copied.digest()) == canonicalHash)
                    },
                )
            }
        }
    ...
    private fun fileReceipt(
        file: File,
        commandHash: CanonicalReceiptBytes,
        fingerprint: CanonicalReceiptBytes,
        header: CanonicalReceiptHeader,
        bodyOffset: Long,
        length: Int,
        canonicalHash: CanonicalReceiptBytes,
    ) = LegacyCanonicalReceipt(
    ...
    private fun skipOwner(data: DataInputStream, group: SurfaceGroup) {
        data.readLong()
    ...
    private fun readSource(data: DataInputStream): PagedSource {
        val id = data.readLong()
        val voxel = Voxel(data.readInt(), data.readInt(), data.readInt())
        val normal = data.readInt()
        val confidence = data.readInt()
        ...
        return PagedSource(
            ...
            normal,
            confidence,
            CanonicalReceiptBytes(fingerprint),
        )
    }

    private fun sourceAt(file: File, offset: Long, count: Int, id: Long): PagedSource? {
        RandomAccessFile(file, "r").use { reader ->
    ...
    internal fun readSourceRecord(reader: RandomAccessFile): PagedSource =
    ...
    private fun snapshotInput(file: File): CountingInputStream =
        CountingInputStream(
            LegacyLimitedInputStream(FileInputStream(file), file.length() - CHECKSUM_BYTES)
        )
...
private data class InlineCanonicalReceipt(
    val command: String,
    val kind: Int,
    val geometry: Long,
    val lineage: Long,
...
private data class CanonicalReceiptHeader(
    val geometry: Long,
    val lineage: Long,
    val high: Long,
    val live: Int,
...
private class LegacyLimitedInputStream(input: InputStream, private var remaining: Long) :
