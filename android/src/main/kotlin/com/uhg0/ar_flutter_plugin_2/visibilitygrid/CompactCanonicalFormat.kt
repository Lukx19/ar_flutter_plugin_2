...
internal data class CompactRoot(
    val high: Long,
    val geometry: Long,
...
internal data class CompactResident(
    val rows: Int,
    val rowId: IntArray,
    val rowX: IntArray,
    val rowY: IntArray,
...
internal class CompactDirectory(capacity: Int) {
    ...
    fun append(entry: CanonicalDirectoryEntry) {
    ...
    fun entry(index: Int) = CanonicalDirectoryEntry(
        CanonicalPageKind.entries.first { it.wire == kind[index].toInt() },
        ordinal[index],
...
internal class CompactPageRanges(capacity: Int) {
    ...
    fun append(location: CompactLocation, first: Int, rows: Int) {
    ...
    fun find(region: StorageRegion, targetPage: Int): Int {
        var low = 0
        var high = size - 1
        while (low <= high) {
...
internal object CompactCanonicalFormat {
    ...
    fun residentFileBytes(legacy: LegacyCanonicalState): Long =
    ...
    fun rootFileBytes(legacy: LegacyCanonicalState): Long {
    ...
    fun writeResident(
        ...
        writeChecked(
            file,
            fault == CompactCanonicalMigrationFault.DURING_RESIDENT_WRITE,
            fault == CompactCanonicalMigrationFault.BEFORE_RESIDENT_SYNC,
        ) { out ->
    ...
    fun writeDirectory(
        ...
        writeChecked(
            file,
            fault == CompactCanonicalMigrationFault.DURING_DIRECTORY_WRITE,
            fault == CompactCanonicalMigrationFault.BEFORE_DIRECTORY_SYNC,
        ) { out ->
    ...
    fun readDirectory(file: File, expected: Int): CompactDirectory {
        verifyChecksum(file)
        require(file.length() <= 1_048_576L)
        checkedInput(file).use { input ->
            ...
            val directory = CompactDirectory(count)
            repeat(count) { index ->
                val wire = input.readUnsignedByte()
                val kind = CanonicalPageKind.entries.firstOrNull { it.wire == wire }
                ...
                require(ordinal == index && length == CanonicalPageCache.PAGE_BYTES)
                require(offset == index.toLong() * length && records in 1..CanonicalPageCache.MAX_RECORDS)
                require(unsignedCompare(minimum, maximum) <= 0)
                directory.append(CanonicalDirectoryEntry(kind, ordinal, minimum, maximum, offset, length, records, hash))
            }
            require(input.read() == -1)
            return directory
        }
    ...
    fun writeRoot(
        ...
        writeChecked(
            file,
            fault == CompactCanonicalMigrationFault.DURING_ROOT_WRITE,
            fault == CompactCanonicalMigrationFault.BEFORE_ROOT_SYNC,
        ) { out ->
            ...
            out.writeInt(CanonicalPageCache.PAGE_BYTES); out.writeInt(CanonicalPageCache.CACHE_PAGES)
    ...
    fun readRoot(
        ...
        checkedInput(file).use { input ->
            ...
            require(input.readInt() == CanonicalPageCache.PAGE_BYTES && input.readInt() == CanonicalPageCache.CACHE_PAGES)
    ...
    private fun checkedInput(file: File): DataInputStream = DataInputStream(
        LimitedInputStream(FileInputStream(file), file.length() - CHECKSUM_BYTES),
    )
