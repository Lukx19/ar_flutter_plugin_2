 ...
 */
internal interface PreparedIntentVisitor {
    fun onHeader(identity: PreparedIntentIdentity): Boolean
    ...
    fun onTerminal(currentReceipt: PreparedIntentCurrentReceipt): Boolean

    companion object {
        val NONE = object : PreparedIntentVisitor {
            ...
            override fun onTerminal(currentReceipt: PreparedIntentCurrentReceipt) = true
        }
    }
}

internal data class PreparedIntentIdentity(
    ...
    val walReceipt: PreparedIntentWalReceipt,
    val expectedCurrentReceipt: PreparedIntentCurrentReceipt,
) {
    companion object {
        fun from(header: DirtyIntentHeader, commandId: String, kind: PreparedMutationKind) = PreparedIntentIdentity(
            ...
            PreparedIntentWalReceipt(header.walLength, header.walHash),
            PreparedIntentCurrentReceipt(header.currentLength, header.currentHash),
        )
    }
}

internal data class PreparedIntentWalReceipt(val length: Long, val hash: CanonicalReceiptBytes)
internal data class PreparedIntentCurrentReceipt(val length: Long, val hash: CanonicalReceiptBytes)
internal sealed interface PreparedIntentIdentityResult {
    data class Complete(val identity: PreparedIntentIdentity) : PreparedIntentIdentityResult
    data class Refused(val reason: PreparedIntentVisitRefusal) : PreparedIntentIdentityResult
}
internal sealed interface PreparedIntentCurrentReceiptResult {
    data class Complete(val receipt: PreparedIntentCurrentReceipt) : PreparedIntentCurrentReceiptResult
    data object Stopped : PreparedIntentCurrentReceiptResult
    data class Refused(val reason: PreparedIntentVisitRefusal) : PreparedIntentCurrentReceiptResult
}
internal sealed interface PreparedIntentVisitResult {
    data class Complete(val identity: PreparedIntentIdentity, val currentReceipt: PreparedIntentCurrentReceipt) : PreparedIntentVisitResult
    data object Stopped : PreparedIntentVisitResult
    data class Refused(val reason: PreparedIntentVisitRefusal) : PreparedIntentVisitResult
}
internal enum class PreparedIntentVisitRefusal {
    CLOSED,
    CORRUPT_INTENT,
    VISITOR_FAILURE,
...
internal object PreparedIntentVisitorResources {
...
internal class PreparedIntentStreamingVisitor(
    ...
    fun visit(
        ...
        if (isClosed()) return refused(PreparedIntentVisitRefusal.CLOSED)
        val header = try { DirtyIntentHeader.read(file) } catch (_: Exception) { return refused(PreparedIntentVisitRefusal.CORRUPT_INTENT) }
        return try {
            FileInputStream(file).use { raw ->
                DataInputStream(BufferedInputStream(BoundedInputStream(raw, header.walOffset, header.walLength), PreparedIntentVisitorResources.STREAMING_SCRATCH_BYTES)).use { input ->
                    val digest = MessageDigest.getInstance("SHA-256")
                    val counter = CountingOutputStream(validatedCurrentOutput)
                    ...
                    val kindOrdinal = input.readInt(); require(kindOrdinal in PreparedMutationKind.entries.indices); output.writeInt(kindOrdinal)
                    val kind = PreparedMutationKind.entries[kindOrdinal]
                    val identity = PreparedIntentIdentity.from(header, command, kind)
                    ...
                    PreparedIntentVisitResult.Complete(identity, receipt)
                }
            }
        } catch (_: PreparedIntentVisitorFailure) {
            refused(PreparedIntentVisitRefusal.VISITOR_FAILURE)
        } catch (_: Exception) {
            refused(if (isClosed()) PreparedIntentVisitRefusal.CLOSED else PreparedIntentVisitRefusal.CORRUPT_INTENT)
        }
    }
    ...
    private fun validCardinality(kind: PreparedMutationKind, header: DirtyIntentHeader, rows: Int, removed: Int, removedSupports: Int, removedLineage: Int, supports: Int, sources: Int, lineage: Int): Boolean {
        ...
        return when (kind) {
            PreparedMutationKind.FEATURE_ADD -> rows == 1 && removed == 0 && supports == 1 && sources == 1 && lineage == 0 && header.targetLive == header.sourceCut.liveSurfaceCount + 1 && header.targetSupport == header.sourceCut.supportCount + 1 && header.targetLineage == header.sourceCut.lineageCount
            PreparedMutationKind.FEATURE_REFINE -> rows == 1 && removed == 0 && supports == 0 && sources == 0 && lineage == 0 && header.targetLive == header.sourceCut.liveSurfaceCount && header.targetSupport == header.sourceCut.supportCount && header.targetLineage == header.sourceCut.lineageCount
            PreparedMutationKind.FEATURE_BATCH -> rows > 0 && removed == 0 && lineage == 0 &&
    ...
    private fun readCommand(input: DataInputStream): String {
        ...
        require(encodedBytes.toLong() in 1..CANONICAL_SURFACE_COMMAND_MODIFIED_UTF_BYTES && input.markSupported())
        input.reset()
        return input.readUTF().also { command ->
            require(validM3CommandId(command) && modifiedUtf8Length(command) == encodedBytes.toLong())
        }
    }
    private fun copyHash(input: DataInputStream, output: java.io.DataOutputStream, expected: CanonicalReceiptBytes) {
    ...
    private fun call(block: () -> Boolean) = !isClosed() && try { block() } catch (_: Exception) { throw PreparedIntentVisitorFailure }
    ...
    private fun stoppedOrClosed() = if (isClosed()) refused(PreparedIntentVisitRefusal.CLOSED) else PreparedIntentVisitResult.Stopped
    private fun refused(reason: PreparedIntentVisitRefusal) = PreparedIntentVisitResult.Refused(reason)
    ...
    private companion object {
        ...
        val LEGACY_DERIVABLE_KINDS = setOf(
            ...
            PreparedMutationKind.FEATURE_REFINE,
            PreparedMutationKind.CREATE,
        )
    }
}

private object PreparedIntentVisitorFailure : RuntimeException()

private class CountingOutputStream(private val delegate: OutputStream? = null) : OutputStream() {
    var count = 0L
