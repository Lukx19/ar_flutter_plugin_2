...
private fun String.asRestoredHashWireIdentity(): String =
    if (isEmpty()) A_EMPTY_HASH_WIRE_IDENTITY else this
...
object StartRequestCodecV2 {
    ...
    sealed interface DetailedDecode {
        ...
        data class Invalid(val failure: ControlValidationFailure) : DetailedDecode
    }

    class ValidationException(val failure: ControlValidationFailure) :
    ...
    fun decodeDetailed(bytes: ByteArray): DetailedDecode {
        fun invalid(error: Int, phase: Int, field: Int, expected: Long, observed: Long) =
            DetailedDecode.Invalid(
                ControlValidationFailure(error, phase, field, expected, observed),
            )
        ...
        if (restoreRequested) {
            // binding lifecycle's first committed transaction has no content-root hashes.
        ...
        return DetailedDecode.Valid(Configuration(
            ...
            modelCapacity,
            pendingCapacity,
            hashIdentity(schemaHash),
            hashIdentity(manifestHash),
            groupFrameConvention,
            matrixConvention,
            directionConvention,
            normalEncoding,
