...
class DurableSessionStoreV2(
    ...
    fun commitStreamed(request: CaptureCommitRequest, streams: List<CaptureComponentStreamV2>): CaptureReceipt = synchronized(mutex) {
        ...
        try {
            ...
            val rootBytes = rootRecord(request, requestHash, revision, staged)
    ...
    private fun rootRecord(request: CaptureCommitRequest, requestHash: String, revision: Long, staged: List<Staged>) = buildString { append("schema=5\nrevision=$revision\nrequest=$requestHash\n"); staged.sortedBy { it.kind.ordinal }.forEach { append("component=${it.kind}:${it.descriptor.byteLength}:${it.descriptor.sha256.hex()}\n") } }.toByteArray(StandardCharsets.UTF_8)
    ...
    private fun selectedRoot(location: File): RootPointer? = listOf("root-A.ptr", "root-B.ptr").mapNotNull { file ->
        File(location, file).takeIf(File::isFile)?.readLines()?.takeIf { it.size == 3 }?.let {
            RootPointer(it[0].toLongOrNull() ?: return@let null, it[1], it[2], if (file == "root-A.ptr") "A" else "B")
        }
    }.maxByOrNull { it.revision }
    private fun publishPointer(location: File, pointer: RootPointer) { val old = selectedRoot(location); val target = File(location, if (old?.slot == "A") "root-B.ptr" else "root-A.ptr"); writeAtomic(target, "${pointer.revision}\n${pointer.rootHash}\n${pointer.commitId}\n".toByteArray()) }
    private fun moveImmutable(from: File, target: File, descriptor: CaptureComponentDescriptor) { if (target.exists()) { check(target.length() == descriptor.byteLength && sha256(target.readBytes()).hex() == descriptor.sha256.hex()) { "Immutable asset conflict" }; from.delete(); return }; try { Files.move(from.toPath(), target.toPath(), StandardCopyOption.ATOMIC_MOVE) } catch (_: AtomicMoveNotSupportedException) { throw IllegalStateException("Atomic asset move unavailable") }; syncDirectory(target.parentFile) }
    private fun copyAndDigest(input: InputStream, output: File): Pair<Long, ByteArray> { val digest = MessageDigest.getInstance("SHA-256"); var length = 0L; DigestInputStream(input, digest).use { source -> FileOutputStream(output).use { destination -> val buffer = ByteArray(64 * 1024); while (true) { val read = source.read(buffer); if (read < 0) break; destination.write(buffer, 0, read); length = Math.addExact(length, read.toLong()) }; destination.fd.sync() } }; syncDirectory(output.parentFile); return length to digest.digest() }
    ...
    private fun writeAtomic(file: File, bytes: ByteArray) { val temporary = File(file.parentFile, ".${file.name}.tmp"); FileOutputStream(temporary).use { it.write(bytes); it.fd.sync() }; try { Files.move(temporary.toPath(), file.toPath(), StandardCopyOption.ATOMIC_MOVE, StandardCopyOption.REPLACE_EXISTING) } catch (_: AtomicMoveNotSupportedException) { throw IllegalStateException("Atomic pointer replace unavailable") }; syncDirectory(file.parentFile) }
    ...
    private data class RootPointer(val revision: Long, val rootHash: String, val commitId: String, val slot: String = "")
}
