...
class CanonicalSourcePagingTest {
    @Test
    fun `source pages preserve unsigned IDs and arbitrary fingerprints in unsigned order`() {
        ...
        val bytes = CanonicalPageCache.encodeSourcePage(7, sources)
    ...
    fun `support page carries exact membership independent of lineage`() {
        val target = SurfaceId(0x8000_0000L)
        val memberships =
            listOf(
                PagedSupport(target, source(1, 1)),
                PagedSupport(target, source(0xffff_ffffL, 2)),
            )
        val decoded =
            CanonicalPageCache.decodeSupports(
                CanonicalPageCache.encodeSupportPage(2, memberships)
            )
    ...
    fun `cache performs at most one fixed read and refuses corruption before publication`() {
        val directory = Files.createTempDirectory("canonical-surface-page-cache").toFile()
        try {
            val file = File(directory, "pages")
            val bytes = CanonicalPageCache.encodeSourcePage(0, listOf(source(1, 0)))
            ...
            CanonicalPageCache(file).use { cache ->
                val first = cache.read(0, entry) as CanonicalPageRead.Complete
                assertEquals(1, first.pageFaults)
                assertEquals(16_384, first.bytesRead)
                val second = cache.read(0, entry) as CanonicalPageRead.Complete
                assertEquals(0, second.pageFaults)
                assertEquals(0, second.bytesRead)
            }
            bytes[100] = 1
            file.writeBytes(bytes)
            val corrupt = CanonicalPageCache(file).use { it.read(0, entry) }
            assertEquals(
                CompactCanonicalRefusal.CORRUPT,
                (corrupt as CanonicalPageRead.Refused).reason,
            )
        } finally {
    ...
    fun `page inspection rejects structurally invalid padding before open publication`() {
        ...
        assertTrue(CanonicalPageCache.inspectPage(entry, bytes) != null)
        bytes[bytes.lastIndex] = 1
        assertEquals(null, CanonicalPageCache.inspectPage(entry, bytes))
    }

    private fun source(id: Long, seed: Int) =
        PagedSource(
            SurfaceId(id),
            Voxel(seed, -seed, seed * 2),
            0x8123,
            197,
            CanonicalReceiptBytes(ByteArray(32) { (seed * 41 + it).toByte() }),
        )
}
